...
void wifi_init_softap() {
    esp_log_level_set("wifi", ESP_LOG_NONE); // Minimize logging
    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    ESP_ERROR_CHECK(esp_wifi_init(&cfg));
    wifi_config_t wifi_config = {
        .ap = {
            .ssid = "ESP8266_SoftAP",
            .ssid_len = strlen("ESP8266_SoftAP"),
            .password = "password",
            .max_connection = 4,
    ...
    ESP_LOGI("wifi_init_softap", "SoftAP set up, SSID:%s password:%s",
             "ESP8266_SoftAP", "password");
}
